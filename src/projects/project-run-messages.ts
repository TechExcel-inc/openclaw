...
export function buildProjectRunContextMessage(
  ...
  return [
    ...
    "Sub-agent delegation (critical for responsiveness): When you need to do long-running or multi-step work—especially browser exploration, website navigation, PFM mapping, form filling, or any task that takes more than ~15 seconds—you MUST spawn a sub-agent using the `sessions_spawn` tool with mode='run'. This keeps your main turn free so you can always respond to operator chat messages. Do NOT do heavy browser/tool work yourself; delegate it to sub-agents. After a sub-agent finishes, its result is announced back to this session—summarize key findings for the operator briefly, then continue.",
